...
export async function experimental_serverChannel(channel: Channel, { configType, presets }: Options) {
        ...
        [
          ...
          channel.on(curEvent, async (data) => {
            try {
              ...
              if (
                EVENTS.SET_CURRENT_STORY === curEvent ||
                EVENTS.CURRENT_STORY_WAS_SET === curEvent
              ) {
                console.log('OUT', curEvent, '.data: ', data);
              }
